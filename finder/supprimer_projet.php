...
if($Auth->user('slug') == 'admin' or $Auth->user('slug') == 'assist') {
	...
	if (isset($_GET['proj']) and isset($_GET['agc']) and isset($_GET['pays'])) {
		$id_proj = $_GET['proj'];
		$id_agc = $_GET['agc'];
		$pays = $_GET['pays'];

		echo $id_proj, $id_agc, $pays;


		//Suppression du prijet depuis la table projet
		global $PDO;
			$req = $PDO->prepare('DELETE FROM projet WHERE id_proj = :Id')
		...
		die('<META HTTP-equiv="refresh" content=0;URL=index.php?p=projets&agence='.$id_agc.'&pays='.$pays.'&msg=oks>');

	}else{
		if (isset($_GET['proj']) and isset($_GET['pole'])) {
			$id_proj = $_GET['proj'];
			$id_pole = $_GET['pole'];
			...
			//Suppression du projet depuis la table projet
			global $PDO;
				$req = $PDO->prepare('DELETE FROM projet WHERE id_proj = :Id')
			...
			die('<META HTTP-equiv="refresh" content=0;URL=index.php?p=projets&pole='.$id_pole.'&msg=oks>');
